...
interface TransportExpenseRecord {
  id: string;
...
export default function TransportExpenseModal({ 
  isOpen, 
  onClose, 
  yearMonth,
  storeId,
  currentStaffList = []
}: TransportExpenseModalProps) {
  const [loading, setLoading] = useState(false);
  const [saving, setSaving] = useState(false);
  const [records, setRecords] = useState<TransportExpenseRecord[]>([]);
  ...
  const loadRecords = async () => {
    try {
      const response = await fetch(
        `/api/transport-expense?year_month=${yearMonth}&store_id=${storeId}`
      );
      ...
      if (data.success && data.records && data.records.length > 0) {
        ...
        setRecords(data.records.map((r: any) => ({
          ...
          transport_expense: r.transport_expense || 0,
  ...
  const updateRecord = (id: string, field: keyof TransportExpenseRecord, value: any) => {
    ...
    try {
      const response = await fetch('/api/transport-expense/save', {
        ...
        body: JSON.stringify({
          ...
          store_id: storeId,
          expenses: validRecords.map(r => ({
            employee_code: r.employee_code,
            employee_name: r.employee_name,
            transport_expense: r.transport_expense,
            expense_notes: r.expense_notes
          }))
        })
      });
  ...
  const totalExpense = records.reduce((sum, r) => sum + (r.transport_expense || 0), 0);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-2">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-6xl h-[98vh] overflow-hidden flex flex-col">
        {/* Header */}
        <div className="px-6 py-3 border-b border-gray-200 flex items-center justify-between bg-gradient-to-r from-blue-500 to-blue-600 text-white flex-shrink-0">
          <div>
            <h2 className="text-xl font-bold">交通費用登記</h2>
            <p className="text-sm text-blue-100 mt-1">月份：{yearMonth}</p>
          </div>
            ...
            <div className="space-y-2 h-full flex flex-col">
              ...
              <div className="flex items-center gap-3 flex-wrap flex-shrink-0">
                ...
                <div className="text-sm text-gray-600">
                  共 {records.length} 筆，總計 ${totalExpense.toLocaleString()}
                </div>
              </div>
                    ...
                    {records.map((record, index) => (
                      <tr key={record.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-3 py-2">
                          <input
                            type="number"
                            value={record.transport_expense || ''}
                            onChange={(e) => updateRecord(record.id, 'transport_expense', parseInt(e.target.value) || 0)}
                            className="w-full px-2 py-1 border border-gray-300 rounded text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                            placeholder="金額"
                            min="0"
                          />
                        ...
                        <td className="px-3 py-2">
                          <input
                            type="text"
                            value={record.expense_notes}
                            onChange={(e) => updateRecord(record.id, 'expense_notes', e.target.value)}
                            className="w-full px-2 py-1 border border-gray-300 rounded text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                            placeholder="必填：交通費用原因"
                          />
                        </td>
