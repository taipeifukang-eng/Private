...
export default function GeneralAffairsServiceCenterPage() {
  ...
  const renderHomeDashboard = () => {
    ...
    const futureInfoCards: Array<{
      ...
      {
        title: '料件動態',
        description: '待料件中心完成後，這裡會顯示申請、庫存不足、調撥與補貨狀態。',
        icon: Package,
        section: 'parts',
        action: '前往料件中心',
      },
          ...
          <div className="grid min-h-[560px] lg:grid-cols-[minmax(0,1fr)_500px]">
            <div className="overflow-auto border-r border-slate-200">
              <table className="w-full min-w-[880px] text-sm">
                ...
                <tbody className="divide-y divide-slate-100">
                  {loadingReports ? (
                    <tr><td colSpan={6} className="px-4 py-10 text-center text-slate-500"><Loader2 className="mx-auto mb-2 animate-spin" />載入中</td></tr>
                  ) : filteredRequests.length === 0 ? (
                    <tr><td colSpan={6} className="px-4 py-10 text-center text-slate-500">目前沒有工單</td></tr>
                  ) : filteredRequests.map((request) => {
                    const normalizedStatus = getNormalizedStatus(request.status);
                    const normalizedStage = getNormalizedStage(request.progress_stage);
                    const meta = statusMeta[normalizedStatus];
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="whitespace-nowrap px-4 py-4 text-xs font-semibold text-slate-500">{getDateTimeLabel(request.reported_at)}</td>
                        <td className="px-4 py-4">
