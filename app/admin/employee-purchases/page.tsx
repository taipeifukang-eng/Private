...
function matchStatusLabel(status: string) {
  if (status === 'employee_code') return '員編比對';
  if (status === 'employee_name') return '姓名比對';
  if (status === 'ambiguous') return '姓名重複';
  return '未比對';
}
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="grid gap-4 lg:grid-cols-[1fr_1.4fr]">
          ...
          <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <div className="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
              ...
              <div className="mt-2 text-xs text-gray-500">下方最多顯示 500 筆明細</div>
            </div>
            <div className="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
        ...
        <section className="rounded-lg border border-gray-200 bg-white shadow-sm">
          <div className="border-b border-gray-200 p-5">
            <h2 className="text-lg font-semibold text-gray-900">購物明細</h2>
            <p className="mt-1 text-sm text-gray-600">
              {selectedPosition ? `目前篩選職稱：${selectedPosition}` : '目前顯示全部職稱'}，金額單位為元。
            </p>
          </div>
          <div className="overflow-auto">
            <table className="min-w-full divide-y divide-gray-200 text-sm">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-3 py-2 text-left font-medium text-gray-600">銷售日</th>
                  <th className="px-3 py-2 text-left font-medium text-gray-600">門市</th>
                  <th className="px-3 py-2 text-left font-medium text-gray-600">會員</th>
                  <th className="px-3 py-2 text-left font-medium text-gray-600">比對同仁</th>
                  <th className="px-3 py-2 text-left font-medium text-gray-600">職稱</th>
                  <th className="px-3 py-2 text-left font-medium text-gray-600">品號 / 品名</th>
                  <th className="px-3 py-2 text-right font-medium text-gray-600">數量</th>
                  <th className="px-3 py-2 text-right font-medium text-gray-600">總金額</th>
              ...
              <tbody className="divide-y divide-gray-100 bg-white">
                {rows.length === 0 ? (
                  ...
                  <tr key={row.id} className="hover:bg-gray-50">
                    <td className="whitespace-nowrap px-3 py-2 text-gray-700">{row.sale_date || '-'}</td>
                    <td className="whitespace-nowrap px-3 py-2 text-gray-700">{row.store_code} {row.store_name}</td>
                    <td className="px-3 py-2 text-gray-700">
                      <div>{row.member_name || '-'}</div>
                      <div className="text-xs text-gray-400">{row.member_code}</div>
                    </td>
                    <td className="px-3 py-2 text-gray-700">
                    ...
                    <td className="min-w-[240px] px-3 py-2 text-gray-700">
                      <div className="font-mono text-xs text-gray-400">{row.product_code}</div>
                      <div>{row.product_name}</div>
                    </td>
                    <td className="whitespace-nowrap px-3 py-2 text-right text-gray-700">{formatDecimal(row.quantity)}</td>
                    <td className="whitespace-nowrap px-3 py-2 text-right font-semibold text-gray-900">{formatMoney(row.total_amount)}</td>
                    <td className="whitespace-nowrap px-3 py-2 text-gray-600">{matchStatusLabel(row.match_status)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
